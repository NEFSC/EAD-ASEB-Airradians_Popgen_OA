...
pdf("C:/Users/samjg/Documents/Github_repositories/EAD-ASEB-Airradians_Popgen_OA/RAnalysis/Output/Popgen/All_Fst.pdf", width = 6, height = 4)
print(all_fst_heatmap)
dev.off()
```
...
F1F2F3_All.vcf.gen <- F1F2F3_All.vcf@gt[,-1]
system.time(F1F2F3_All.gen_table <- matrix(convertVCFtoCount3(F1F2F3_All.vcf.gen), ncol=ncol(F1F2F3_All.vcf.gen)))
# user  system elapsed 
# 0.57    0.00    1.38

# finally, we are here
F1F2F3_All.SNPdata <- t(F1F2F3_All.gen_table) # transpose, gen_table has columns as indivs, rows as loci, switch

############################ #
# locusNames ############### #
############################ #

F1F2F3_All_locinames <- paste(F1F2F3_All.vcf@fix[,"CHROM"], 
                           F1F2F3_All.vcf@fix[,"POS"], sep="_")
length(F1F2F3_All_locinames) == nrow(F1F2F3_All.gen_table)

############################ #
# popNames - genlight object was created in line 157 from F1F2F3_LOW_MOD.vcf 
############################ #

All_gen@strata # grab population from here, first make sure its the same order as the matrix above
# LowvMod_gen@strata$ID == rownames(LowvMod_matrix) # all MUST BE TRUE! good to go
table(All_gen@strata$Population) # call LowvMod_gen@strata$Population in ```MakeDiploidFSTMat```
# F1_Low F1_Moderate      F2_Low F2_Moderate      F3_Low F3_Moderate 
#     58          55          32          46          49          49
```
...
F1F2F3_All.FSTMat <- OutFLANK::MakeDiploidFSTMat(F1F2F3_All.SNPdata, 
                                                 locusNames = F1F2F3_All_locinames, 
                                                 popNames = All_gen@strata$Population)# MakeDiploidFSTMat(SNPmat,locusNames,popNames)
head(F1F2F3_All.FSTMat) # view first few rows
...
```{r, Check He vs. Fst}
plot(F1F2F3_All.FSTMat$He, F1F2F3_All.FSTMat$FST)
```
...
plot(F1F2F3_All.FSTMat$FST, F1F2F3_All.FSTMat$FSTNoCorr)
abline(0,1)
