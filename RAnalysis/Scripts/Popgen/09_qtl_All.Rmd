...
# check out the output!
length(unique(lme4qtl.mergedalleles.output$locus)) # 1600 - about 60%, what about the other SNPs? why did they not run? check in the next chunk
...
raw_qtl.df <- read.csv("RAnalysis/Output/Popgen/qtl/All_QTL_raw.csv", sep = ",", head = T) %>% select(!X)
...
length(unique(raw_qtl.df$locus)) # 1600
