...
Pedtest <- function(GenoM, LifeHistData, threshold.LLR, min.LLR) { # start of fxn
  ...
  x <- data.frame(seq(10, 100, by=5))# to 100 every 10
...
for (i in 1:nrow(x)) { # start of i for loop
  ...
  pedigree.raw$num_SNPs      <- num_SNPs
  pedigree.raw$uniq_pedigree <- uniq_pedigree
  pedigree.raw$mean_LRRpair  <- mean_LRRpair

  
  pedtest.out <- rbind(pedtest.out, pedigree.raw) %>% na.omit()
  ...
  loess.plot  <- ggplot(pedtest.out, aes(x=num_SNPs)) +
                    geom_smooth(aes(y=uniq_pedigree), method="loess", col="grey60") +
                    geom_smooth(aes(y=mean_LRRpair * scaleFactor), method="loess", col="grey20") +
