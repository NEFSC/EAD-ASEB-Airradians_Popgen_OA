...
F0BF1O.vcf <- All.vcf[,c(1:3, 5:10, 12:15, 17:26,63:139, 27:62)] # 138 samples, 2,947 variants, 16 chromosomes
...
* what to do next? 
- optimize the number of input SNPs based on maximum output of assigned pedigrees

## Pedtest

**Objective** Run a function ```Pedtest``` that iterates ```sequoia``` from 1:100 
outputting the number of unique assigned pedigrees. Create a figure with x = N SNPs and y = assigned pedigree
theretically if this works we should either see data top out at a number of SNPs or yield a bell-like curve 
where the peak is the desired input 

  - why a function? We should repeat this test for each pedigree group (ie F0 + F1, F1_low_Broodstock + F12_low_Offpsring, etc.)

```{r Pedtest}

Pedtest <- function(GenoM, LifeHistData, threshold.LLR, min.LLR) { # start of fxn
  
  x <- data.frame(seq(10, 70, by=2))# to 100 every 10

  pedtest.raw           <- data.frame(matrix(ncol = 3))
  colnames(pedtest.raw) <- c('num_SNPs', 'uniq_pedigree', 'mean_LRRpair')
  
  pedtest.out           <- data.frame(matrix(ncol = 3)) # create dataframe to save cumunalitively during for loop
  colnames(pedtest.out) <- c('num_SNPs', 'uniq_pedigree', 'mean_LRRpair') # names for comuns in the for loop
  
  # for loop for number of SNPs, repeating sequoia for each
for (i in 1:nrow(x)) { # start of i for loop
  
  num_SNPs      <- x[i,] # the main call here, 1:100 repeating this loop
  randomSNPs    <- array(sample(c(sample(1:ncol(GenoM), num_SNPs, replace = TRUE)))) # call random n.SNPs from the matrix
  reduced_GenoM <- GenoM[,randomSNPs] # truncate the GenoM by the i number of randomSNPs
  
  run.sequoia <- sequoia(GenoM = reduced_GenoM,
  ...
  df.loop       <- data.frame(run.sequoia$PedigreePar %>% select(id, LLRpair) %>% na.omit())
  uniq_pedigree <- nrow(df.loop)
  mean_LRRpair <- mean(df.loop$LLRpair)
  
  
  pedtest.raw$num_SNPs      <- num_SNPs
  pedtest.raw$uniq_pedigree <- uniq_pedigree
  pedtest.raw$mean_LRRpair  <- mean_LRRpair

  
  pedtest.out <- rbind(pedtest.out, pedtest.raw) %>% na.omit()
  
      } # end of i for loop

  # plot it 
  
  xint        <- (pedtest.out %>% filter(mean_LRRpair == max(mean_LRRpair)))$num_SNPs
  scaleFactor <- max(pedtest.out$uniq_pedigree) / max(pedtest.out$mean_LRRpair)
  loess.plot  <- ggplot(pedtest.out, aes(x=num_SNPs)) +
                    geom_smooth(aes(y=uniq_pedigree), method="loess", col="grey60", se=F) +
                    geom_smooth(aes(y=mean_LRRpair * scaleFactor), method="loess", col="grey20", se=F) +
                    scale_y_continuous(name="uniq_pedigree", sec.axis=sec_axis(~./scaleFactor, name="mean_LRRpair")) +
                    geom_vline(xintercept = xint) + 
                    theme_bw()

  return(list(output  = pedtest.out,
              plot    = loess.plot))
...
```{r pedigree function 'SequoiaBootPed'}

SequoiaBootPed <- function(GenoM, LifeHistData, threshold.LLR, min.LLR, n.SNPs, n.bootstrap) {
...
if ( length(unique(pedigree.summary$id)) < offspring.n) {# TRUE = not enough bootstrap runs 
    ...
    print(paste0("Predigree summary: ",
                 ...
                 "/",
                 nrow(pedigree.final),
                 " unique pedigree assignments are TRUE fertilized pairs"))
  
    # print(paste0("Edit the n.SNP or n.bootstrap call for better pedigree"))
    print(paste0("Run GetMaybeRelBootPed to fill the gaps!"))
...
```{r GetMaybeRelBootPed}

GetMaybeRelBootPed <- function(Sequoia_pedigree, 
                               GenoM, 
                               LifeHistData, 
                               # threshold.LLR, 
                               # min.LLR, 
                               n.SNPs 
                               # n.bootstrap # old version ran static number of times, scratch this run until finished
                               ) {
  
  # file to wroite getmayberel outputs to
  getmaybetrio_raw           <- data.frame(matrix(ncol = 6)) # create dataframe to save cumunalitively during for loop
  colnames(getmaybetrio_raw) <- c('id', 'parent1', 'parent2', 'LLRpair', 'n.loci', 'loci') # names for comuns in the for loop
  # mock output file for the contingency statement
  pedigree_out               <- data.frame(matrix(ncol = 6))
  colnames(pedigree_out)     <-c('id', 'dam', 'sire', 'LLRpair', 'n.loci', 'loci')

  # old version ran a static number of times, scratch this we want to run until finished
  # for (i in 1:n.bootstrap) { # n.bootstrap, number of times the for loop is iterated
    
  # criteria to understand how many offspring are not assigned pedigree to keep looping through GetMaybeRel
  offspring.n <- nrow( # the number of UNIQUE ids needed in the pedigree_out file to be done!
                      LifeHistData %>% 
                        dplyr::filter(!(BirthYear == min(LifeHistData$BirthYear) |  # omit occurnaces of F0s (if any!)
                                        id %in% Sequoia_pedigree$id) # AND those already in the pedigree
                                      )
                     )
  

  if (nrow(pedigree_out) < offspring.n) {# TRUE, gaps to fill meaning offspring without pedigree assignment, run GetMaybeRel 
    # NOte: as long as we need more than 1 unique id for pedigree (the mock pedigree_out matrix has 1 blank row initially) then we are good for this version
    
    # IMPORTTAT the while loop repeadely runs the data until pedigree_out has at least the number of rows as offspring.n
    while (nrow(pedigree_out) < offspring.n) {
          ...
          SNP_IDs         <- paste0(colnames(bootstrap_GenoM), collapse=", ")
          #un getmayberel
          run.getmayberel <- GetMaybeRel(
                                  GenoM = bootstrap_GenoM, # F0BF1O.GenoM_final,
                                  SeqList = NULL,
                                  Pedigree = as.data.frame(Sequoia_pedigree %>% #F0BF1O.pedigree.final %>% 
                                                             select(id, dam, sire)),
                                  LifeHistData = LifeHistData,#F0BF1O.LifeHistData,
                                  AgePrior = NULL,
                                  Module = "par",
                                  Complex = "full",
                                  Herm = "A",
          ...
          #run.getmayberel$MaybePar # view it
                  # add results from getmayberel to cumulative table 
                  if (length(run.getmayberel$MaybeTrio) > 0) { # if the length of the output is > 0 (results!) do the following
                      df_loop <- data.frame(run.getmayberel$MaybeTrio %>% 
                                             dplyr::select(id, parent1, parent2, LLRpair) %>%  
                                             # na.omit()) %>% 
                                             mutate(loci   = SNP_IDs,
                                                    n.loci = ncol(bootstrap_GenoM)))
                     getmaybetrio_raw <- rbind(getmaybetrio_raw, df_loop) %>% na.omit() #bind to a cumulative list dataframe
                     
                     
                     maybetrio_out <- getmaybetrio_raw %>% # get unique ids for the top hit of LLRpair
                                           group_by(id) %>% 
                                           top_n(1, LLRpair) 

                     parent1 <- maybetrio_out %>%  ungroup() %>% dplyr::select(parent1) %>% unique()
                     parent2 <- maybetrio_out %>%  ungroup() %>% dplyr::select(parent2) %>% unique()
                              
                     parent1.sex <- merge(parent1, 
                                         (LifeHistData %>%  
                                                dplyr::select(id, Sex) %>% 
                                                dplyr::filter(id %in% 
                                                                parent1$parent1) %>% 
                                                dplyr::rename(parent1=id)), by = 'parent1') %>% 
                                                dplyr::rename(sex.parent1 = Sex)
                              
                              
                     parent2.sex <- merge(parent2, 
                                         (LifeHistData %>%  
                                               dplyr::select(id, Sex) %>% 
                                               dplyr::filter(id %in% parent2$parent2) %>% 
                                               dplyr::rename(parent2=id)), by = 'parent2') %>% 
                                               dplyr::rename(sex.parent2 = Sex)
                              
                      maybetrio_out.sex_1   <- merge(maybetrio_out, parent1.sex)
                      maybetrio_out.sex_1_2 <- merge(maybetrio_out.sex_1, parent2.sex)
                              
                      pedigree_loop <- maybetrio_out.sex_1_2 %>% 
                                              dplyr::mutate(dam  = # assign parent 1 or parent 2 as dam based on criteria below (1 = dam, female)
                                                             case_when(
                                                                      ((sex.parent1 == 1 | sex.parent1 == 4) & sex.parent2 != 1) ~ parent1,
                                                                      ((sex.parent2 == 1 | sex.parent2 == 4) & sex.parent1 != 1) ~ parent2),
                                                            sire = # assign parent 1 or parent 2 as sire based on criteria below (2 = sire, male)
                                                             case_when(
                                                                      (dam == parent1) ~ parent2,
                                                                      (dam == parent2) ~ parent1)) %>% 
                                              dplyr::select(id, dam, sire, LLRpair, n.loci, loci)   
                      pedigree_out <- rbind(pedigree_out, pedigree_loop)  %>% #bind to a cumulative list dataframe
                                            na.omit() %>% # remove NA (at the start of making the matrix first row is NA
                                            dplyr::group_by(id) %>% # group by id, objective to have one output id per row
                                            dplyr::top_n(1, LLRpair) %>% # if multiple ids, take the one with top LLR for the trio pedigree
                                            distinct(id, .keep_all = TRUE) # possible to have the same LLRpair value and multiple ids,here delete ,multiples taking one
                      
                   } else { # there is no output from this run of getmaybe rel
                     # < nothing here, if GetMaybe rel has no output just run again! >
                     # <getmaybetrio_raw will be NA >
                   } # close the if else for any output of getmayberel
          
    } # close the while loop
    
  # } # end of for statement the sole purpose is to fire up the while loop
  } else { # length(unique(getmaybetrio_raw$id)) < offspring.n
   # if this works correctly, we no proceed to return() given that we now ran getmayberel enough to get pedigrees
  }
# OUTSIDE FOR LOOP
                
return(list(raw = getmaybetrio_raw,
            out  = pedigree_out))
...
# SequoiaBootPed(GenoM, LifeHistData, outputstring)
...
* run SequoiaBootPed for all low and moderate animals F0 through F3
...
```{r SequoiaBootPed run all low and moderate individuals}


# F0 AND F1 OFFPSRING ALL   ::::::::::::::::::::::::::::::::::::::::::
All_Low_Mod.sequoia <- SequoiaBootPed(
...
output: 
[1] "Predigree summary: 102/102 offspring with pedigree assignment; 83/102 TRUE fertilized pairs"
[1] "Proceed, all offspring have at least 1 pedigree assignment"

```{r Pedigree construction: F0BF1O}


F0BF1O.Pedtest <- Pedtest(F0BF1O.GenoM_final, F0BF1O.LifeHistData, -2, 0.2)
F0BF1O.Pedtest$out # view the end product data, numb of SNPs and LLR results
F0BF1O.Pedtest$plot # iew - vertical line x int is the highest mean LLR, not related to # pedigrees assigned

pdf("RAnalysis/Output/Popgen/pedigree/F0BF1O/F0BF1O_Pedtest.pdf", width = 4, height = 4)
print(F0BF1O.Pedtest$plot)
dev.off()
...
# output
write.csv(F0BF1O.pedigree.raw, "RAnalysis/Output/Popgen/pedigree/F0BF1O/F0BF1O_raw.csv")
write.csv(F0BF1O.pedigree.summary, "RAnalysis/Output/Popgen/pedigree/F0BF1O/F0BF1O_summary.csv")
...
rownames(F0BF1O.pedigree.final) <- NULL # renomve rownames
nrow(F0BF1O.pedigree.final) # 102  DO NOT NEED TO RUN GETMAYBEREL!
# write.csv
write.csv(F0BF1O.pedigree.final, "RAnalysis/Output/Popgen/pedigree/F0BF1O/F0BF1O_final.csv")


```


* run SequoiaBootPed for F1BF2J LOW

inputs:
  - GenoM = F1BF2O_LOW.GenoM_final
  - LifeHistData = F1BF2O_LOW.LifeHistData
  - threshold.LLR = -2
  - min.LLR = 0.1
  - n.SNP = 50
  - n.bootstrap = 100
...
```{r Pedigree construction: F1BF2O LOW}


F1BF2O_LOW.Pedtest <- Pedtest(F1BF2O_LOW.GenoM_final, F1BF2O_LOW.LifeHistData, -2, 0.2)
F1BF2O_LOW.Pedtest$out
F1BF2O_LOW.Pedtest$plot


pdf("RAnalysis/Output/Popgen/pedigree/F1BF2O/Low/F1BF2O_LOW_Pedtest.pdf", width = 4, height = 4)
print(F1BF2O_LOW.Pedtest$plot)
dev.off()
...
# low
F1BF2O_LOW.sequoia <- SequoiaBootPed(
...
# out - top LLR taking unique IDs
F1BF2O_LOW.pedigree.out  <- as.data.frame(F1BF2O_LOW.pedigree.summary) %>% 
                            group_by(id) %>% # group by id - there are mutliple occurances of pedigree assign for ids!
                            # filter(Fertilization_pairs %in% 'TRUE') %>% 
                            dplyr::top_n(1, LLRpair ) %>%  # take the best hit essential, highest LLR total 
                            distinct(id, .keep_all = TRUE) %>% 
                            dplyr::select(id, dam, sire, LLRpair , dam.batch, sire.batch, Fertilization_pairs, loci) # select the target columns for next sequoia run
rownames(F1BF2O_LOW.pedigree.out) <- NULL # renomve rownames

#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# which ids did not get pedigree???
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

F1BF2O_LOW.LifeHistData %>% filter(!(BirthYear %in% min(BirthYear) |
                                        id %in% F1BF2O_LOW.pedigree.out$id))
# 6 rows, 6 ids did not, run et mayRel


#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# Run GetMaybeRelBootPed 
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

F1BF2O_LOW.getmayberel <- GetMaybeRelBootPed(F1BF2O_LOW.pedigree.out, #sequoia pedigree output final
                                             F1BF2O_LOW.GenoM_final, # GenoM, 
                                             F1BF2O_LOW.LifeHistData, # LifeHistData, 
                                             50#  n.SNPs, 
                                             )
F1BF2O_LOW.getmayberel.out <- as.data.frame(F1BF2O_LOW.getmayberel$out)

#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# output
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

write.csv(F1BF2O_LOW.pedigree.raw, "RAnalysis/Output/Popgen/pedigree/F1BF2O/Low/F1BF2O_Low_SequoiaBootPed_raw.csv")
write.csv(F1BF2O_LOW.pedigree.summary, "RAnalysis/Output/Popgen/pedigree/F1BF2O/Low/F1BF2O_Low_SequoiaBootPed_summary.csv")
write.csv(F1BF2O_LOW.pedigree.out, "RAnalysis/Output/Popgen/pedigree/F1BF2O/Low/F1BF2O_Low_SequoiaBootPed_out.csv")
write.csv(F1BF2O_LOW.getmayberel.out, "RAnalysis/Output/Popgen/pedigree/F1BF2O/Low/F1BF2O_Low_GetMaybeRelBootPed_out.csv")


```

* run SequoiaBootPed for F1BF2J MODERATE

inputs:
  - GenoM = F1BF2O_MOD.GenoM_final
  - LifeHistData = F1BF2O_MOD.LifeHistData
  - threshold.LLR = -2
  - min.LLR = 0.1
  - n.SNP = 50
  - n.bootstrap = 100
...
output: 
 *  run on 2/7/2025
[1] "Predigree summary: 35/46 offspring with pedigree assignment; 10/35 pedigree assignments with highest abs LLR are TRUE fertilized pairs"
[1] "Edit the n.SNP or n.bootstrap call for better pedigree"

```{r Pedigree construction:  F1BF2O MODERATE}


F1BF2O_MOD.Pedtest <- Pedtest(F1BF2O_MOD.GenoM_final, F1BF2O_MOD.LifeHistData, -2, 0.2)
F1BF2O_MOD.Pedtest$out
F1BF2O_MOD.Pedtest$plot

pdf("RAnalysis/Output/Popgen/pedigree/F1BF2O/Moderate/F1BF2O_MOD_Pedtest.pdf", width = 4, height = 4)
print(F1BF2O_MOD.Pedtest$out)
dev.off()
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# F1 AND F2 OFFPSRING (treatment specific)   
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
# moderate
F1BF2O_MOD.sequoia <- SequoiaBootPed(
...
# run on 1/11/2025
# [1] "Predigree summary: 38/46 offspring with pedigree assignment; 18/38 pedigree assignments with highest abs LLR are TRUE fertilized pairs"
# [1] "Edit the n.SNP or n.bootstrap call for better pedigree"
...
# out - top LLR taking unique IDs
F1BF2O_MOD.pedigree.out  <- as.data.frame(F1BF2O_MOD.pedigree.summary) %>% 
                            group_by(id) %>% # group by id - there are mutliple occurances of pedigree assign for ids!
                            # filter(Fertilization_pairs %in% 'TRUE') %>% 
                            dplyr::top_n(1, LLRpair ) %>%  # take the best hit essential, highest LLR total 
                            distinct(id, .keep_all = TRUE) %>% 
                            dplyr::select(id, dam, sire, LLRpair , dam.batch, sire.batch, Fertilization_pairs, loci) # select the target columns for next sequoia run
rownames(F1BF2O_MOD.pedigree.out) <- NULL # renomve rownames

#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# which ids did not get pedigree???
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

F1BF2O_MOD.LifeHistData %>% filter(!(BirthYear %in% min(BirthYear) |
                                        id %in% F1BF2O_MOD.pedigree.out$id))
# 11 rows, 11 ids did not, run et mayRel
...
F1BF2O_MOD.getmayberel <- GetMaybeRelBootPed(F1BF2O_MOD.pedigree.out, #sequoia pedigree output final
                                             F1BF2O_MOD.GenoM_final, # GenoM, 
                                             F1BF2O_MOD.LifeHistData, # LifeHistData, 
                                             50#  n.SNPs, 
                                             )

F1BF2O_MOD.getmayberel.out <- as.data.frame(F1BF2O_MOD.getmayberel$out)

#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# output
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

write.csv(F1BF2O_MOD.pedigree.raw, "RAnalysis/Output/Popgen/pedigree/F1BF2O/Moderate/F1BF2O_MOD_SequoiaBootPed_raw.csv")
write.csv(F1BF2O_MOD.pedigree.summary, "RAnalysis/Output/Popgen/pedigree/F1BF2O/Moderate/F1BF2O_MOD_SequoiaBootPed_summary.csv")
write.csv(F1BF2O_MOD.pedigree.out, "RAnalysis/Output/Popgen/pedigree/F1BF2O/Moderate/F1BF2O_MOD_SequoiaBootPed_out.csv")
write.csv(F1BF2O_MOD.getmayberel.out, "RAnalysis/Output/Popgen/pedigree/F1BF2O/Moderate/F1BF2O_MOD_GetMaybeRelBootPed_out.csv")

```

* run SequoiaBootPed for F1BF2J HIGH

inputs:
  - GenoM = F1BF2O_HIGH.GenoM_final
  - LifeHistData = F1BF2O_HIGH.LifeHistData
  - threshold.LLR = -2
  - min.LLR = 0.1
  - n.SNP = 50
  - n.bootstrap = 100
...
output: 
 *  run on 2/7/2025
[1] "Pedigree summary: 24/33 offspring with pedigree assignment; 10/24 unique pedigree assignments are TRUE fertilized pairs"
[1] "Run GetMaybeRelBootPed to fill the gaps!"

```{r Pedigree construction: F1BF2O HIGH}



F1BF2O_HIGH.Pedtest <- Pedtest(F1BF2O_HIGH.GenoM_final, F1BF2O_HIGH.LifeHistData, -2, 0.2)
F1BF2O_HIGH.Pedtest$out
F1BF2O_HIGH.Pedtest$plot

pdf("RAnalysis/Output/Popgen/pedigree/F1BF2O/High/F1BF2O_HIGH_Pedtest.pdf", width = 4, height = 4)
print(F1BF2O_HIGH.Pedtest$plot)
dev.off()
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# F1 AND F2 OFFPSRING (treatment specific)   
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
# high
F1BF2O_HIGH.sequoia <- SequoiaBootPed(
...
# out - top LLR taking unique IDs
F1BF2O_HIGH.pedigree.out  <- as.data.frame(F1BF2O_HIGH.pedigree.summary) %>% 
                            group_by(id) %>% # group by id - there are mutliple occurances of pedigree assign for ids!
                            # filter(Fertilization_pairs %in% 'TRUE') %>% 
                            dplyr::top_n(1, LLRpair ) %>%  # take the best hit essential, highest LLR total 
                            distinct(id, .keep_all = TRUE) %>% 
                            dplyr::select(id, dam, sire, LLRpair , dam.batch, sire.batch, Fertilization_pairs, loci) # select the target columns for next sequoia run
rownames(F1BF2O_HIGH.pedigree.out) <- NULL # renomve rownames

#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# which ids did not get pedigree???
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

F1BF2O_HIGH.LifeHistData %>% filter(!(BirthYear %in% min(BirthYear) |
                                        id %in% F1BF2O_HIGH.pedigree.out$id))
# 9 rows, 9 ids did not, run et mayRel
...
F1BF2O_HIGH.getmayberel <- GetMaybeRelBootPed(F1BF2O_HIGH.pedigree.out, #sequoia pedigree output final
                                             F1BF2O_HIGH.GenoM_final, # GenoM, 
                                             F1BF2O_HIGH.LifeHistData, # LifeHistData, 
                                             50#  n.SNPs, 
                                             )

F1BF2O_HIGH.getmayberel.out <- as.data.frame(F1BF2O_HIGH.getmayberel$out)

#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# output
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

write.csv(F1BF2O_HIGH.pedigree.raw, "RAnalysis/Output/Popgen/pedigree/F1BF2O/High/F1BF2O_HIGH_SequoiaBootPed_raw.csv")
write.csv(F1BF2O_HIGH.pedigree.summary, "RAnalysis/Output/Popgen/pedigree/F1BF2O/High/F1BF2O_HIGH_SequoiaBootPed_summary.csv")
write.csv(F1BF2O_HIGH.pedigree.out, "RAnalysis/Output/Popgen/pedigree/F1BF2O/High/F1BF2O_HIGH_SequoiaBootPed_out.csv")
write.csv(F1BF2O_HIGH.getmayberel.out, "RAnalysis/Output/Popgen/pedigree/F1BF2O/High/F1BF2O_HIGH_GetMaybeRelBootPed_out.csv")
```


* run BootPed for F2BF3J LOW

inputs:
  - GenoM = F2BF3O_LOW.GenoM_final
  - LifeHistData = F2BF3O_LOW.LifeHistData
  - threshold.LLR = -2
  - min.LLR = 0.1
  - n.SNP = 50
  - n.bootstrap = 100
...
output: 
 *  run on 2/7/2025
[1] "Pedigree summary: 41/49 offspring with pedigree assignment; 8/41 unique pedigree assignments are TRUE fertilized pairs"
[1] "Run GetMaybeRelBootPed to fill the gaps!"
  
```{r Pedigree construction: F2BF3O LOW}



F2BF3O_LOW.Pedtest <- Pedtest(F2BF3O_LOW.GenoM_final, F2BF3O_LOW.LifeHistData, -2, 0.2)
F2BF3O_LOW.Pedtest$out
F2BF3O_LOW.Pedtest$plot

pdf("RAnalysis/Output/Popgen/pedigree/F2BF3O/Low/F2BF3O_LOW_Pedtest.pdf", width = 4, height = 4)
print(F2BF3O_LOW.Pedtest$plot)
dev.off()


#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# F3 OFFPSRING (treatment specific)   
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

# low
F2BF3O_LOW.sequoia <- SequoiaBootPed(
...
# out - top LLR taking unique IDs
F2BF3O_LOW.pedigree.out  <- as.data.frame(F2BF3O_LOW.pedigree.summary) %>% 
                            group_by(id) %>% # group by id - there are mutliple occurances of pedigree assign for ids!
                            # filter(Fertilization_pairs %in% 'TRUE') %>% 
                            dplyr::top_n(1, LLRpair ) %>%  # take the best hit essential, LOWest LLR total 
                            distinct(id, .keep_all = TRUE) %>% 
                            dplyr::select(id, dam, sire, LLRpair , dam.batch, sire.batch, Fertilization_pairs, loci) # select the target columns for next sequoia run
rownames(F2BF3O_LOW.pedigree.out) <- NULL # renomve rownames

#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# which ids did not get pedigree???
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

F2BF3O_LOW.LifeHistData %>% filter(!(BirthYear %in% min(BirthYear) |
                                        id %in% F2BF3O_LOW.pedigree.out$id))
# 8 rows, 8 ids did not, run et mayRel
...
F2BF3O_LOW.getmayberel <- GetMaybeRelBootPed(F2BF3O_LOW.pedigree.out, #sequoia pedigree output final
                                             F2BF3O_LOW.GenoM_final, # GenoM, 
                                             F2BF3O_LOW.LifeHistData, # LifeHistData, 
                                             50#  n.SNPs, 
                                             )

F2BF3O_LOW.getmayberel.out <- as.data.frame(F2BF3O_LOW.getmayberel$out)

#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# output
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

write.csv(F2BF3O_LOW.pedigree.raw, "RAnalysis/Output/Popgen/pedigree/F2BF3O/Low/F2BF3O_LOW_SequoiaBootPed_raw.csv")
write.csv(F2BF3O_LOW.pedigree.summary, "RAnalysis/Output/Popgen/pedigree/F2BF3O/Low/F2BF3O_LOW_SequoiaBootPed_summary.csv")
write.csv(F2BF3O_LOW.pedigree.out, "RAnalysis/Output/Popgen/pedigree/F2BF3O/Low/F2BF3O_LOW_SequoiaBootPed_out.csv")
write.csv(F2BF3O_LOW.getmayberel.out, "RAnalysis/Output/Popgen/pedigree/F2BF3O/Low/F2BF3O_LOW_GetMaybeRelBootPed_out.csv")

```


* run BootPed for F2BF3J MODERATE

inputs:
  - GenoM = F2BF3O_MOD.GenoM_final
  - LifeHistData = F2BF3O_MOD.LifeHistData
  - threshold.LLR = -2
  - min.LLR = 0.1
  - n.SNP = 50
  - n.bootstrap = 100
...
output: 
 *  run on 2/7/2025
[1] "Predigree summary: 45/49 offspring with pedigree assignment; 7/45 unique pedigree assignments are TRUE fertilized pairs"
[1] "Run GetMaybeRelBootPed to fill the gaps!"

```{r Pedigree construction: F2BF3O MODERATE}



F2BF3O_MOD.Pedtest <- Pedtest(F2BF3O_MOD.GenoM_final, F2BF3O_MOD.LifeHistData, -2, 0.2)
F2BF3O_MOD.Pedtest$out
F2BF3O_MOD.Pedtest$plot


pdf("RAnalysis/Output/Popgen/pedigree/F2BF3O/Moderate/F2BF3O_MOD_Pedtest.pdf", width = 4, height = 4)
print(F2BF3O_MOD.Pedtest$plot)
dev.off()
...
# moderate
F2BF3O_MOD.sequoia <- SequoiaBootPed(
...
# out - top LLR taking unique IDs
F2BF3O_MOD.pedigree.out  <- as.data.frame(F2BF3O_MOD.pedigree.summary) %>% 
                            group_by(id) %>% # group by id - there are mutliple occurances of pedigree assign for ids!
                            # filter(Fertilization_pairs %in% 'TRUE') %>% 
                            dplyr::top_n(1, LLRpair ) %>%  # take the best hit essential, MODest LLR total 
                            distinct(id, .keep_all = TRUE) %>% 
                            dplyr::select(id, dam, sire, LLRpair , dam.batch, sire.batch, Fertilization_pairs, loci) # select the target columns for next sequoia run
rownames(F2BF3O_MOD.pedigree.out) <- NULL # renomve rownames

#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# which ids did not get pedigree???
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

F2BF3O_MOD.LifeHistData %>% filter(!(BirthYear %in% min(BirthYear) |
                                        id %in% F2BF3O_MOD.pedigree.out$id))
# 4 rows, 4 ids did not, run et mayRel
...
F2BF3O_MOD.getmayberel <- GetMaybeRelBootPed(F2BF3O_MOD.pedigree.out, #sequoia pedigree output final
                                             F2BF3O_MOD.GenoM_final, # GenoM, 
                                             F2BF3O_MOD.LifeHistData, # LifeHistData, 
                                             50#  n.SNPs, 
                                             )

F2BF3O_MOD.getmayberel.out <- as.data.frame(F2BF3O_MOD.getmayberel$out)

#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# output
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

write.csv(F2BF3O_MOD.pedigree.raw, "RAnalysis/Output/Popgen/pedigree/F2BF3O/Moderate/F2BF3O_MOD_SequoiaBootPed_raw.csv")
write.csv(F2BF3O_MOD.pedigree.summary, "RAnalysis/Output/Popgen/pedigree/F2BF3O/Moderate/F2BF3O_MOD_SequoiaBootPed_summary.csv")
write.csv(F2BF3O_MOD.pedigree.out, "RAnalysis/Output/Popgen/pedigree/F2BF3O/Moderate/F2BF3O_MOD_SequoiaBootPed_out.csv")
write.csv(F2BF3O_MOD.getmayberel.out, "RAnalysis/Output/Popgen/pedigree/F2BF3O/Moderate/F2BF3O_MOD_GetMaybeRelBootPed_out.csv")


```

* run BootPed for F2BF3J HIGH

inputs:
  - GenoM = F2BF3O_HIGH.GenoM_final
  - LifeHistData = F2BF3O_HIGH.LifeHistData
  - threshold.LLR = -2
  - min.LLR = 0.1
  - n.SNP = 50
  - n.bootstrap = 100
...
output: 
 *  run on 2/7/2025
[1] "Predigree summary: 40/44 offspring with pedigree assignment; 11/40 pedigree assignments with highest abs LLR are TRUE fertilized pairs"
[1] "Edit the n.SNP or n.bootstrap call for better pedigree"

```{r Pedigree construction: F2BF3O HIGH}



F2BF3O_HIGH.Pedtest <- Pedtest(F2BF3O_HIGH.GenoM_final, F2BF3O_HIGH.LifeHistData, -2, 0.2)
F2BF3O_HIGH.Pedtest$out
F2BF3O_HIGH.Pedtest$plot

pdf("RAnalysis/Output/Popgen/pedigree/F2BF3O/High/F2BF3O_HIGH_Pedtest.pdf", width = 4, height = 4)
print(F2BF3O_HIGH.Pedtest$plot)
dev.off()
...
# out - top LLR taking unique IDs
F2BF3O_HIGH.pedigree.out  <- as.data.frame(F2BF3O_HIGH.pedigree.summary) %>% 
                            group_by(id) %>% # group by id - there are mutliple occurances of pedigree assign for ids!
                            # filter(Fertilization_pairs %in% 'TRUE') %>% 
                            dplyr::top_n(1, LLRpair ) %>%  # take the best hit essential, HIGHest LLR total 
                            distinct(id, .keep_all = TRUE) %>% 
                            dplyr::select(id, dam, sire, LLRpair , dam.batch, sire.batch, Fertilization_pairs, loci) # select the target columns for next sequoia run
rownames(F2BF3O_HIGH.pedigree.out) <- NULL # renomve rownames

#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# which ids did not get pedigree???
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

F2BF3O_HIGH.LifeHistData %>% filter(!(BirthYear %in% min(BirthYear) |
                                        id %in% F2BF3O_HIGH.pedigree.out$id))
# 4 rows, 4 ids did not, run et mayRel
...
F2BF3O_HIGH.getmayberel <- GetMaybeRelBootPed(F2BF3O_HIGH.pedigree.out, #sequoia pedigree output final
                                             F2BF3O_HIGH.GenoM_final, # GenoM, 
                                             F2BF3O_HIGH.LifeHistData, # LifeHistData, 
                                             50#  n.SNPs, 
                                             )

F2BF3O_HIGH.getmayberel.out <- as.data.frame(F2BF3O_HIGH.getmayberel$out)

#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# output
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

write.csv(F2BF3O_HIGH.pedigree.raw, "RAnalysis/Output/Popgen/pedigree/F2BF3O/High/F2BF3O_HIGH_SequoiaBootPed_raw.csv")
write.csv(F2BF3O_HIGH.pedigree.summary, "RAnalysis/Output/Popgen/pedigree/F2BF3O/High/F2BF3O_HIGH_SequoiaBootPed_summary.csv")
write.csv(F2BF3O_HIGH.pedigree.out, "RAnalysis/Output/Popgen/pedigree/F2BF3O/High/F2BF3O_HIGH_SequoiaBootPed_out.csv")
write.csv(F2BF3O_HIGH.getmayberel.out, "RAnalysis/Output/Popgen/pedigree/F2BF3O/High/F2BF3O_HIGH_GetMaybeRelBootPed_out.csv")
```
...
gff <- ape::read.gff("C:/Users/samjg/Documents/Bioinformatics/refs/Airradians/GCF_041381155.1_genomic.gff") %>% 
          ...
          dplyr::select(chromosme, start, end, transcript_id, protein_id)
