...
bind_wtl_results <- data.frame(rbind(raw_qtl.df, raw_qtl_merged_altalleles.df))


Test <- All_master_reordered_rm %>% dplyr::select(!unique(bind_wtl_results$locus))
...
m1.anova     <- anova(m1) # does not get the p value
m1.df        <- m1.anova$npar[3]
m1.Fvalue    <- m1.anova$`F value`[3] # gen 
m1.pvalue    <- stats::pf(m1.Fvalue, m1.df, 271, lower.tail = FALSE) 
...
```{r lme4qtl run with merged alt SNPs}

# read in 
lme4qtl.output <- read.csv("RAnalysis/Output/Popgen/qtl/All/All_QTL_raw.csv")
...
raw_qtl.df                   <- read.csv("RAnalysis/Output/Popgen/qtl/All/results/All_QTL_raw.csv", sep = ",", head = T) %>% 
                                        select(!X)
raw_qtl_merged_altalleles.df <- read.csv("RAnalysis/Output/Popgen/qtl/All/results/All_QTL_raw_merged_altalleles.csv", sep = ",", head = T) %>%
                                        select(!X)

length(
  unique(
    (rbind(raw_qtl.df,raw_qtl_merged_altalleles.df))$locus
    ))# 2657 - this is the total number of loci from QTL for F1-F3 low moderate and high
...
# whereas the normal bonferroni correct by dividing pvalues by the sum of all tests (2,657 in our case!)
...
# example: model with the lowest pvales are adjust p dividing by 1, the next lowest by 2 and so on...
...
# make a dataframe adding the column 'rank' from 1 to length loci_order and rename loci_order to locus to merge in the next step
...
Bonferroni_qtl.df <- raw_qtl.df.sorted %>% 
                            ...
                            select(chromosome, chrom_num, position, locus, effect, 
                                   df, Fvalue, pvalue,rank, adj_bonferroni, sig_boolean)
                                   


# save output 
write.csv(Bonferroni_qtl.df, "RAnalysis/Output/Popgen/qtl/All_results/All_QTL_bonferroni.csv")
...
Bonferroni_qtl.df     <- read.csv("RAnalysis/Output/Popgen/qtl/All/results/All_QTL_bonferroni.csv", sep = ",", head = T) %>% select(!X)

length(
  unique(
    (Bonferroni_qtl.df)$locus
    ))# 2657 - this is the total number of loci from QTL for F1-F3 low moderate and high
...
nrow(loci_interaction.df) # 13 (7 without merging alt alleles in the second run)
...
loci_generation_only.df  <- significant_loci.df %>% filter(!(locus %in% loci_interaction.df$locus | # gen * treatment, remove all loci
                                                          locus %in% loci_treatment.df$locus)) # treatment, remove all loci
nrow(loci_generation_only.df) # 47 (31 without merging alt alleles in the second run)
...
loci_treatment_only.df <- significant_loci.df %>% filter(!(locus %in% loci_interaction.df$locus | # gen * treatment, remove all loci
                                                          locus %in% loci_generation_only.df$locus)) # gen only, remove all loci
nrow(loci_treatment_only.df)
# 22 (16 wihtout merging alt alleles in the second run)

# save output 
write.csv(loci_interaction.df, "RAnalysis/Output/Popgen/qtl/All/results/All_QTL_bonferroni_genXtreatment.csv")
write.csv(loci_generation_only.df, "RAnalysis/Output/Popgen/qtl/All/results/All_QTL_bonferroni_gen_only.csv")
write.csv(loci_treatment.df, "RAnalysis/Output/Popgen/qtl/All/results/All_QTL_bonferroni_treatment_only.csv")
...
Bonferroni_TRUE  <- unique(Bonferroni_qtl.df %>%  
                             dplyr::select(locus, sig_boolean) %>%  
                             dplyr::rename(SNP = locus) %>% 
                             dplyr::filter(sig_boolean %in% 'TRUE'))
Bonferroni_FALSE <- unique(Bonferroni_qtl.df %>%  
                             dplyr::select(locus, sig_boolean) %>%  
                             dplyr::rename(SNP = locus) %>% 
                             dplyr::filter(sig_boolean %in% 'FALSE'))
qtl_boolean.df   <- rbind(Bonferroni_TRUE, 
                          (Bonferroni_FALSE %>% filter(!SNP %in% Bonferroni_TRUE$SNP ))
                          )
nrow(qtl_boolean.df) # 2657 PERFECT!
...
qtl_chromosome_boolean.df <- merge(qtl_chromosome.df, # merge the qtl chromosome, wider with cumulative position of SNP and each main effect
                                   ...
                                   by = "SNP")
nrow(qtl_chromosome_boolean.df) # 2656 PERFECT! Note that this sig_boolean doesnt indiviate WHICH effect is sig, just that the snp has a sig effect
...
qtl_chromosome_annotated <- unique(
                                  ...
                                  select(SNP, Chromosome, Position, 
                                         'generation * treatment', generation, treatment, 
                                         location, protein_id, sig_boolean)
...
qtl_chromosome_annotated_exon   <- qtl_chromosome_annotated %>% 
                                          filter(location %in% "exon")
qtl_chromosome_annotated_intron <- qtl_chromosome_annotated %>% 
                                          filter(location %in% 'intron') %>% 
                                          filter(!SNP %in% qtl_chromosome_annotated_exon$SNP)
# now rbind them
...
CMplot(qtl_manhattan_treatment,
       ...
       # threshold=c(1e-6,1e-4),
       threshold=c(0.00051,1e-6), # 0.001
       cir.chr.h=1.5,
       amplify=TRUE,
       threshold.lty=c(1,2),
       ...
       highlight.cex = 1.5,
       highlight.col = "black",
       file.name="All_Manhattan_Treatment",
       dpi=300,
       file.output=TRUE,
       verbose=TRUE,
       width=10,
...
CMplot(qtl_manhattan_generation,
       ...
       chr.labels=paste("Chr",c(1:16),sep=""),
       # threshold=c(1e-6,1e-4),
       threshold=c(0.00061,1e-6), # 0.001
       cir.chr.h=1.5,
       amplify=TRUE,
       threshold.lty=c(1,2),
       threshold.col=c("#999999","#999999"),
       ...
       highlight.cex = 1.5,
       highlight.col = "black",
       file.name="All_Manhattan_Generation",
       dpi=300,
       file.output=TRUE,
       verbose=TRUE,
       width=10,
...
CMplot(qtl_manhattan_treatmentXgeneration,
       ...
       chr.labels=paste("Chr",c(1:16),sep=""),
       # threshold=c(1e-6,1e-4),
       threshold=c(0.00049,1e-6), # 0.001
       cir.chr.h=1.5,
       amplify=TRUE,
       threshold.lty=c(1,2),
       threshold.col=c("#999999","#999999"),
       ...
       highlight.cex = 1.5,
       highlight.col = "black",
       file.name="All_Manhattan_TreatmentXGeneration",
       dpi=300,
       file.output=TRUE,
       verbose=TRUE,
       width=10,
