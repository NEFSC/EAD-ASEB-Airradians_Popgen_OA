...
raw_qtl.df <- read.csv("RAnalysis/Output/Popgen/qtl/All/All_QTL_raw.csv", sep = ",", head = T) %>% 
                            select(!X)
raw_qtl_merged_altalleles.df <- read.csv("RAnalysis/Output/Popgen/qtl/All/All_QTL_raw_merged_altalleles.csv", sep = ",", head = T) %>% select(!X)
...
# example f the cirvular manhattan plot 
...
# rad the reference data, objetive here to call the 'rgion' of start and end for chromosome size
...
# rchromosome regions only
chromosome_regions.df <- reference.df %>% 
                              dplyr::filter(type %in% 'region') %>% 
                              select(!c(type,transcript_id, protein_id))
for (i in 2:nrow(chromosome_regions.df)) { # now cange the start point to be +1bp of the previous chromosome end point
  chromosome_regions.df$start[i] = chromosome_regions.df$end[(i-1)] + 1
}


# edi the bonferroni correct qtl results to merge with chromosome_regions.df
Bonferroni_qtl.df  <- Bonferroni_qtl.df %>% mutate(chromosome = paste0(chromosome,".1"))

# merge with chromosome_regions.df
Bonferroni_qtl_manhattan.df <- merge(chromosome_regions.df, 
                                     Bonferroni_qtl.df, 
                                     by = c("chromosome", "chrom_num")) %>% 
                           mutate(locus = paste0(chromosome,'_',position),
                          ...
                          tidyr::pivot_wider(id_cols = c("SNP","Chromosome","Position"),
                                             names_from = "effect",
                                             values_from = "pvalue") %>% 
                          select(SNP, Chromosome, Position, treatment)

CMplot(Bonferroni_qtl_manhattan.df,
       type="p",
       plot.type="c",
       r=0.4,
       col=c("grey30","grey60"),
...
CMplot(Bonferroni_qtl_manhattan.df,type="p",plot.type="c",LOG10=TRUE,outward=TRUE,col=matrix(c("#4DAF4A",NA,NA,"dodgerblue4",
         "deepskyblue",NA,"dodgerblue1", "olivedrab3", "darkgoldenrod1"), nrow=3, byrow=TRUE),
         chr.labels=paste("Chr",c(1:16),sep=""),threshold=NULL,r=1.2,cir.chr.h=1.5,axis.cex=1,
         cir.band=1,file="jpg", file.name=NULL,dpi=300,chr.den.col="black",file.output=TRUE,verbose=TRUE,
         width=10,height=10)

loci <- F1F2F3_All.FSTMat %>% mutate(chromosome = gsub("_.*", "\\1", LocusName),
                              position = gsub(".*_", "", LocusName)) %>% select(chromosome, 
                                                                                           position)

loci_master <- merge(loci, chromosome_regions.df) %>% 
                    mutate(locus = paste0(chromosome,'_',position),
                           position = as.numeric(position) + as.numeric(start),
                           ) %>% 
                    rename(SNP = locus,
...
out_trim.2 <- out_trim$results  %>% 
                      rename(SNP = LocusName) %>%  
                      select(SNP, FST, pvalues, qvalues, OutlierFlag)
F1F2F3_All.fstMaster <- merge(loci_master, out_trim.2, by = "SNP")
library(qqman)
gwasResults

# Make the Manhattan plot on the gwasResults dataset
manhattan( (F1F2F3_All.fstMaster %>% na.omit()), chr="CHR", bp="BP", snp="SNP", p="pvalues")

# what are the outliers? 
View(F1F2F3_All.fstMaster)
View( out_trim$results)
# CM084278.1_20591212 and CM084278.1_24948349
```
