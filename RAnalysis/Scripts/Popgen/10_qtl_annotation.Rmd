...
all_qtl_interaction <- read.csv("RAnalysis/Output/Popgen/qtl/All_QTL_bonferroni_genXtreatment.csv", sep = ',') %>%  select (-X)
all_qtl_generation  <- read.csv("RAnalysis/Output/Popgen/qtl/All_QTL_bonferroni_gen_only.csv", sep = ',') %>%  select (-X)
all_qtl_treatment   <- read.csv("RAnalysis/Output/Popgen/qtl/All_QTL_bonferroni_treatment_only.csv", sep = ',') %>%  select (-X)
...
ggplot(data = ind_coords.Qtl_interaction, aes(x = Axis1, y = Axis2))+
  ...
  # centroids
  geom_label(data = centroid.Qtl_interaction, aes(label = Site, fill = Site), size = 4, show.legend = FALSE)+
  # colouring
  scale_fill_manual(values = cols.Qtl_interaction)+
  scale_colour_manual(values = cols.Qtl_interaction)+
  # custom labels
