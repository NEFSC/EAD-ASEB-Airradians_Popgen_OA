...
View(lme4qtl.output)
# save output 
write.csv(lme4qtl.output, "RAnalysis/Output/Popgen/qtl/LowvMod_QTL_raw.csv")
```
...
# read in 
lme4qtl.output <- read.csv("RAnalysis/Output/Popgen/qtl/LowvMod_QTL_raw.csv")
...
# save output 
# save output 
write.csv(lme4qtl.mergedalleles.output, "RAnalysis/Output/Popgen/qtl/LowvMod_QTL_raw_merged_altalleles.csv")
...
raw_qtl.df    <- read.csv("RAnalysis/Output/Popgen/qtl/LowvMod_QTL_raw.csv", sep = ",", head = T) %>% 
                                select(!X)
raw_merged_altalleles_qtl.df <- read.csv("RAnalysis/Output/Popgen/qtl/LowvMod_QTL_raw_merged_altalleles.csv", sep = ",", head = T) %>%
                                      select(!X)
...
# save output 
write.csv(Bonferroni_qtl.df, "RAnalysis/Output/Popgen/qtl/LowvMod_QTL_bonferroni.csv")
...
Bonferroni_qtl.df     <- read.csv("RAnalysis/Output/Popgen/qtl/LowvMod_QTL_bonferroni.csv", sep = ",", head = T) %>% select(!X)
...
nrow(loci_complex.df) # 9 (6 without the second run of merged alt alleles)
...
nrow(loci_captivity.df) # 41 (25 without the second run of merged alt alleles)



# (3) Selective mortatlity (Bottleneck) effect: treatment only suggest that the initial F1 exposure truncated the population to these heritable loci
...
# 16 (10 without the second run of merged alt alleles)

# save output 
write.csv(loci_complex.df, "RAnalysis/Output/Popgen/qtl/LowvMod_QTL_bonferroni_genXtreatment.csv")
write.csv(loci_captivity.df, "RAnalysis/Output/Popgen/qtl/LowvMod_QTL_bonferroni_gen_only.csv")
write.csv(loci_bottleneck.df, "RAnalysis/Output/Popgen/qtl/LowvMod_QTL_bonferroni_treatment_only.csv")
...
exon.df %>% 
  left_join(loci_complex.df, by = c("chromosome","chrom_num")) %>% 
  # filter(position >= start & position <= end) %>% # exact position
  filter(position >= start-2000 & position <= end+2000) %>% 
  select(chromosome, chrom_num, position, locus, protein_id, start, end)

exon.df %>% 
  left_join(loci_bottleneck.df, by = c("chromosome","chrom_num")) %>% 
  # filter(position >= start & position <= end) %>% # exact position
  filter(position >= start-2000 & position <= end+2000) %>% 
  select(chromosome, chrom_num, position, locus, protein_id, start, end)
...
exon.df %>% 
  left_join(loci_captivity.df, by = c("chromosome","chrom_num")) %>% 
  filter(position >= start & position <= end) %>% # exact position
  # filter(position >= start-2000 & position <= end+2000) %>% 
  select(chromosome, chrom_num, position, locus, protein_id, start, end) %>%  distinct()
```

```{r Manhattan plots}

View(Bonferroni_qtl.df)
test <- Bonferroni_qtl.df %>% 
                             dplyr::filter(effect %in% 'generation * treatment') %>% 
                             mutate(chromosome = gsub("^(CM\\d+).*", "\\1", locus),
                                    position   = as.numeric(gsub(".*_(\\d+)\\.\\d+$", "\\1", locus))) %>% 
                             mutate(chrom_num = 
                                      case_when(chromosome  %in% 'CM084264' ~1, 
...
data <- data.frame(
  ...
  Chromosome = c(rep("chr1", 100), rep("chr2", 150), rep("chr3", 200)), 

  Position = c(seq(1, 100, by = 1), seq(1, 150, by = 1), seq(1, 200, by = 1)), 

  pvalue = runif(450, 0, 1)
...
data <- test %>% 
  ...
  mutate(cum_pos = position + lag(cumsum(position)), default = 0) %>% 
...
ggplot(data, aes(x = position, y = -log10(pvalue), shape = sig_boolean, color = sig_boolean)) + 
  ...
  facet_grid(~ chrom_num, scales = "free_x", space = "free") + 
